...
\diagcode{E084}, \diagcode{E101}--\diagcode{E130} & Modules, attributes, generics, packing, ADT access, effects, verification \\
...
\diagcode{E011}
  ...
  & \textit{impure operation in pure function '\texttt{f}'} ---
    Issued when a \lain{func} body contains a call to a \lain{proc}; a
    \lain{while} loop whose measure is neither given nor inferable; a direct
    self-recursion for which no measure can be inferred; or any mutual
    recursion.  (A direct recursion \emph{with} a well-founded measure is
    permitted; a present-but-failing measure is \diagcode{E082}/\diagcode{E091}.)
    \\[4pt]
...
\diagcode{E013}
  ...
  & \textit{redeclaration or shadowing of '\texttt{x}' is forbidden} ---
    ...
    an outer binding, a \lain{main} declared as \lain{func}, or a binding
...
\diagcode{E082}
  ...
  & \textit{termination measure not proven to decrease} ---
    Issued when the compiler cannot verify that every looping-back path
    ...
    smaller and keeps it non-negative.
    \\[4pt]
...
\diagcode{E106} & use of an undeclared identifier (diagnosed at translation-output generation) \\[2pt]
...
\diagcode{E124} & generic instantiation: non-generic type, missing or uninferable type argument, wrong arity \\[2pt]
...
\diagcode{E126} & a mutable borrow (\lain{var}) stored in a struct field or bound to a local --- a reference lives only for the call that creates it \\[2pt]
...
\diagcode{E130} & a declared \lain{effects} row does not cover what the body does \\[2pt]
\diagcode{E131} & an \lain{extern} declares a slice parameter --- a slice has no C representation; pass a pointer and a length \\[2pt]
\diagcode{E132} & a struct field's refinement names another field --- relational field invariants are not implemented, and one that is ignored is an unpaid assume \\[2pt]
\end{longtable}
