...
Mutual recursion                  & forbidden          & allowed \\
...
\begin{constraint}
  A \lain{func} that contains a \lain{while} loop whose termination the
...
\begin{constraint}
  A \lain{func} that participates in a \emph{mutual}-recursion cycle is
  \illformed{}~(\diagcode{E011}).  The implementation shall detect mutual
  recursion by performing a depth-first search on the call graph.
\end{constraint}
