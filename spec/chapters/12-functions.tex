...
\begin{constraint}
  ...
  \illformed{}~(\diagcode{E100}).  If a future revision introduces
  return-by-reference, it will require dedicated syntax.
\end{constraint}

\begin{note}
  When the function's declared return type is owned (e.g.\ \lain{mov T}),
  the expression in \lain{return expr} is treated as a move of \lain{expr}.
  No explicit \lain{mov} keyword is permitted in return position.
\end{note}
