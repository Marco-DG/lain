...
\pnum
A procedure declaration introduces a \lain{proc}: a callable whose effects are
granted by its introducer rather than by its row.  \textbf{This form is
deprecated.}  Every effect a \lain{proc} was needed for is now stated by an
\lain{effects} row on an ordinary \lain{func}, so the keyword grants nothing that
the row does not, and a \lain{proc} whose row is empty draws
\diagcode{W130}.  It is retained only until the corresponding function
\emph{type} syntax lands.
