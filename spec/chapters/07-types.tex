...
length followed by a data pointer, in that order.  Because a slice carries
its own length it is \emph{not} a raw pointer, and so --- unlike the thin
pointer forms below --- it takes \emph{no} \lain{*} prefix.

\begin{note}
  The spelling \lain{*T[]} is accepted as a synonym for \lain{T[]}: the two
  denote the same fat-slice type and emit identical C.  The \lain{*}-prefixed
  forms that carry no runtime length (\lain{*T[N]}, \lain{*T[:S]}, \lain{*T})
  are \textit{thin} views; the bare bracket forms (\lain{T[]}, \lain{T[n]})
  are fat.  \lain{T[]} is the canonical slice spelling.
\end{note}
...
\lain{T[]}       & fat slice, runtime length         & usize + ptr \\
\lain{*T[N]}     & thin view, compile-time length    & ptr only \\
\lain{*T[:S]}    & thin view, sentinel-terminated    & ptr only \\
