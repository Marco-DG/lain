...
Owned.  When the value is of a \emph{linear} type, an Owned local must be
consumed before its scope exits (\diagcode{E003}); a local of an unrestricted
type carries no such obligation.
...
\begin{constraint}
  Using a variable, or a reference whose owner has been moved, after that
  value has been transferred (use-after-move; the Consumed state) is
  \illformed{}~(\diagcode{E001}).  Using a variable \emph{before} it is
  initialized --- the Uninit state --- is a different diagnostic,
  \diagcode{E005} (below).
\end{constraint}

\begin{constraint}
  Consuming a struct field that has already been consumed (a double-consume
  at field granularity) is \illformed{}~(\diagcode{E002}).
\end{constraint}
...
\begin{constraint}
  Reading a variable, struct field, or array element before it has been
  initialized (use of an uninitialized value; the Uninit state) is
  \illformed{}~(\diagcode{E005}).  Definite-assignment analysis tracks
  initialization per binding and per field.
\end{constraint}

\begin{constraint}
  Consuming a linear variable or field inside a loop body that may execute
  more than once is \illformed{}~(\diagcode{E006}).
\end{constraint}

\begin{constraint}
  Passing a linear value to an owned (\lain{mov}) parameter without the
  explicit \lain{mov} keyword at the call site --- an implicit move --- is
  \illformed{}~(\diagcode{E007}).  (Consuming a linear value on some branches
  but not others is a separate diagnostic, \diagcode{E016}, below.)
\end{constraint}
