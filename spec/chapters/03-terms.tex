...
\index{func}
A function.  Its effects are those its \textbf{effect row} names, and the row's
default is empty --- pure and terminating.  A function shall not have an effect
its row does not name (see \S\,\ref{sec:12-functions}).

\subsection*{purity}
\index{purity}
The property of a function with an empty row: its result depends only on its inputs and it
has no observable side effects on any state outside its own stack frame.

\subsection*{effect row}
\index{effect row}
The set of effects a function may have, drawn from \lain{io}, \lain{diverge},
\lain{raises} and \lain{alloc}.  Inferred from the body, empty by default, widened
by an \lain{effects} clause, and a complete upper bound in either case
(see \S\,\ref{sec:fn.distinction}).

\subsection*{\texttt{proc}}
\index{proc}
\emph{Removed.}  An introducer that granted effects rather than stating them; the
effect row replaced it.  The keyword is still reserved and diagnoses the
replacement (see \S\,\ref{sec:decl.proc}).
