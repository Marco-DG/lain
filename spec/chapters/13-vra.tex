...
\pnum
Value Range Analysis (VRA) is the static analysis that establishes, at every
program point, sound facts about the integer values --- and the relationships
between values --- that may arise at run time.  It is the mechanism by which a
conforming implementation discharges the language's compile-time proof
obligations, and thereby its safety guarantees and check-free code generation.
It is used to:
\begin{itemize}
  \item prove array and slice accesses in bounds (\S\,\ref{sec:types.array});
  \item prove the absence of division/modulo by zero (\S\,\ref{sec:expr.arith});
  \item prove the absence of arithmetic overflow at a checked operation
        (\S\,\ref{sec:vra.boundary});
  \item verify parameter and return refinements, and propagate a return
        refinement to call sites (\S\,\ref{sec:vra.constraints});
  \item establish the loop and recursion facts that make termination measures
        provable (\S\,\ref{sec:fn.termination}).
\end{itemize}

\pnum
\textbf{This clause specifies the \emph{contract} of the VRA --- what a
conforming implementation shall guarantee --- not the abstract domain or
algorithm used to achieve it.}  The domain and algorithm are an implementation
concern; the reference design (a relational abstract interpreter over the
octagon domain, run as a fixpoint over a control-flow graph) is given in the
companion VRA design annex.  Any analysis satisfying the contract below
conforms, whether it reasons with intervals, difference bounds, octagons, or a
stronger relational domain.

\begin{constraint}
  \textbf{Soundness.}
  The VRA shall compute, at each program point, an \emph{over-approximation} of
  the set of concrete states reachable there: every state that can occur at run
  time shall be admitted by the analysis result at that point.  Consequently a
  proof obligation the VRA reports as \emph{discharged} holds on every
  execution --- an accepted access is in bounds, an accepted operation does not
  overflow or divide by zero, and a verified refinement holds for every result.
  A proof obligation the implementation cannot discharge shall be reported as an
  error, never silently assumed (\emph{prove-or-reject}).
\end{constraint}

\begin{constraint}
  \textbf{Determinism and decidability.}
  The VRA shall be deterministic and shall terminate on every input, in time
  polynomial in program size.  No general SMT, Presburger, or unbounded search
  procedure is required or permitted in the analysis core; a conforming
  implementation reaches its verdict by a decidable, terminating computation.
\end{constraint}

\section{What the analysis tracks}
\label{sec:vra.range}
\index{range representation}

\pnum
The analysis maintains, at each program point, a sound over-approximation of
each live integer value's possible values and --- for a relational
implementation --- of the relationships between values (for example
$i < \mathit{len}$, $\mathit{lo} \leq \mathit{hi}$, or $o = i + j$).  The
per-value component is an interval $[\ell,\, u]$ (with $-\infty$, $+\infty$ for
an absent bound, and \textit{unknown} $= (-\infty, +\infty)$); the relational
component, when present, records linear relations among values.  The concrete
representation is implementation-defined; only the soundness contract above is
normative.
...
\pnum
\textbf{Equality and disequality.}  A test \lain{x == v} refines \lain{x} to
the point range $[v, v]$ in the then-branch.  A disequality (the else-branch,
or an explicit \lain{x != v}) refines \lain{x} only when $v$ lies \emph{at a
boundary} of the range $[\ell, u]$ --- to $[v+1, u]$ when $v = \ell$, or to
$[\ell, v-1]$ when $v = u$ --- since removing an interior point cannot be
expressed as a single interval.  In particular an unsigned \lain{n} guarded
by \lain{if n == 0 \{ return \}} is refined to $[1, u]$ on the fall-through,
so a subsequent \lain{a[0]} on a length-\lain{n} slice is in bounds.

\pnum
\textbf{Unsigned subtraction.}  For $r = a - b$ at an \emph{unsigned} result
type, the relation $r = a - b$ holds over $\mathbb{Z}$ but not over the type
unless $a \ge b$: the difference wraps to a large positive value otherwise.  An
implementation shall record no relation between $r$, $a$ and $b$ unless it can
show $a \ge b$; where it can --- \lain{hi - lo} under a live \lain{lo < hi}, or
\lain{n - 1} behind \lain{if n == 0 \{ return \}} --- the relation is exact and
may be recorded.

\begin{note}
  Recording the $\mathbb{Z}$ relation unconditionally is unsound and not merely
  imprecise.  Under it, \lain{hi = mid - 1} with \lain{mid} zero appears to give
  \lain{hi} the value $-1$ rather than the type's maximum, and every bound
  derived from that relation inherits the error.
\end{note}

\pnum
\textbf{Early-return refinement.}  When an \lain{if} whose then-branch exits
the enclosing scope (via \lain{return}, and provided no \lain{break} escapes)
has condition \textit{C}, the code that follows the \lain{if} is reached only
when \textit{C} is false; the VRA therefore refines with $\lnot\textit{C}$
for the remainder of the scope.  Likewise, after a \lain{while cond \{ ... \}}
loop whose body contains no \lain{break} at that loop's level, the VRA
assumes $\lnot\mathit{cond}$, since the loop exits only when the condition
fails (e.g.\ after \lain{while j > 0 \{ ... \}}, \lain{j} is refined to $0$).

\pnum
\textbf{Length guards.}  A guard on a slice or array length --- \lain{i <
a.len}, and also \lain{a.len > 0}, \lain{a.len >= k}, \lain{a.len != 0} ---
refines the underlying length.  For a fixed array \lain{a T[N]} the length is
the constant $N$; for a sized slice \lain{a T[n]} it is the size parameter
\lain{n}.  Thus \lain{if a.len > 0 \{ a[0] \}} is accepted exactly as
\lain{if n > 0 \{ a[0] \}} would be.
...
\begin{enumerate}
  ...
  \item VRA does not track ranges across loop-carried dependences with
        non-linear measures.
  \item The analysis reasons over $\mathbb{Z}$: its arithmetic does not wrap.
        This is consistent with the language, because an operation that could
        overflow is \illformed{}~(\S\,\ref{sec:lex.overflow}) and the operators
        that \emph{do} wrap or saturate are written explicitly and carry no
        obligation.  A program whose correctness depends on wrapping shall say
        so with \tok{+\%}; the analysis then makes no claim about the value.
