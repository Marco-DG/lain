...
\pnum
Plain arithmetic operators (\tok{+}, \tok{-}, \tok{*}, \tok{/}) on
sized integer values whose static range cannot be proven to fit the
target type are \illformed{}~(\diagcode{E086}). Explicit operators
indicate the cases where overflow is intentional: \tok{+\%},
\tok{-\%}, \tok{*\%} (wrapping) and \tok{+|}, \tok{-|}, \tok{*|}
(saturating). See \S\,\ref{sec:vra.boundary}.
