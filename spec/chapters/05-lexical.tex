...
\gramref{token} is given in Annex~A (\S\,\ref{sec:lex.tokens}).
...
\pnum
A semicolon (\tok{;}) may be used as a statement separator, equivalent to a
line feed.  Its purpose is to place more than one statement or declaration on a
single line; a newline is the normal terminator, and no semicolon is needed at
the end of a line.  Two statements on one line without a separator are
ill-formed~(\diagcode{E100}).

\begin{rationale}
  Newlines as the primary terminator eliminate the class of bugs caused by
  \emph{missing} terminators.  A semicolon is available only when the programmer
  deliberately wants several statements on one line (e.g.\ \lain{a = 1; b = 2}),
  making a multi-statement line an explicit, opt-in choice rather than something
...
\tok{break}       & Loop break                     & \S\,\ref{sec:09-statements} \\
\tok{c\_include}  & Foreign header include         & \S\,\ref{sec:17-interop} \\
\tok{case}        & Pattern matching               & \S\,\ref{sec:15-match} \\
\tok{comptime}    & Compile-time evaluation        & \S\,\ref{sec:14-generics} \\
\tok{continue}    & Loop continue                  & \S\,\ref{sec:09-statements} \\
...
\tok{extern}      & Foreign function declaration   & \S\,\ref{sec:17-interop} \\
...
\tok{proc}        & \emph{reserved}; removed, see \diagcode{E100} & \S\,\ref{sec:decl.proc} \\
\tok{effects}     & Effect row                     & \S\,\ref{sec:fn.distinction} \\
\tok{return}      & Return statement               & \S\,\ref{sec:09-statements} \\
\tok{true}        & Boolean literal                & \S\,\ref{sec:lex.literals} \\
\tok{type}        & Type / struct / enum declaration & \S\,\ref{sec:10-declarations} \\
\tok{unsafe}      & Unsafe block                   & \S\,\ref{sec:18-unsafe} \\
\tok{use}         & Local-scope import / alias     & \S\,\ref{sec:16-modules} \\
\tok{var}         & Mutable binding / mutable borrow & \S\,\ref{sec:10-declarations} \\
\tok{while}       & Loop statement                 & \S\,\ref{sec:09-statements} \\
\end{longtable}

\begin{constraint}
  An identifier that is identical to a keyword shall not be used in any
  context that requires an identifier.  Such use is
...
\begin{constraint}
  ...
  context is \illformed{}~(\diagcode{E086}) --- it is diagnosed by the same
  range mechanism that rejects arithmetic overflow (\S\,\ref{sec:vra.boundary}).
\end{constraint}
...
var c u8 = 256      // [E086]: 256 does not fit in u8
...
\tok{/}  & Division \\
\tok{\%} & Modulo (remainder) \\
\tok{+?} \tok{-?} \tok{*?} & Checked add/sub/mul (\lain{panic} on overflow) \\
\tok{+\%} \tok{-\%} \tok{*\%} & Wrapping add/sub/mul (modular) \\
\tok{+|} \tok{-|} \tok{*|} & Saturating add/sub/mul (clamp) \\
\end{longtable}

\pnum
The plain \tok{+}, \tok{-}, \tok{*} auto-widen their result so they cannot
overflow (\S\,\ref{sec:types.rank}); the \tok{?}/\tok{\%}/\tok{|} variants keep
the operand type and select an explicit overflow policy
...
\pnum
Attributes use bracket syntax \tok{[name]} or \tok{[name(args)]} before
a declaration (\S\,\ref{sec:10-declarations}).  The brackets \tok{[}
and \tok{]} listed above serve both as array/slice subscripts and as
attribute delimiters; the context (declaration position vs.\ expression
position) determines the role.
...
\bottomrule
\endlastfoot
\tok{*} & Multiplication (binary)            & Pointer dereference (unary prefix, only in \tok{unsafe}) \\
...
\pnum
A program in which an integer operation can overflow is \illformed{}.  The
plain operators \tok{+}, \tok{-} and \tok{*} carry a proof obligation: the
result shall be shown to lie within its type.  Where the implementation
cannot show it, the program is \illformed{}~(\diagcode{E086}).  Overflow is
therefore a diagnosable constraint violation, not a runtime behaviour.

\pnum
\tok{+}, \tok{-} and \tok{*} \emph{widen}: the result of an operation on two
operands of width $N < 64$ has a type wide enough to represent every result,
so the operation itself cannot overflow.  The obligation attaches instead to
the \emph{narrowing} --- the point at which that result is stored into, passed
as, or returned as a narrower type.  At width 64 no wider type exists and the
obligation attaches to the operation.

\pnum
Overflow that is intended shall be written as such.  Each form is a distinct
operator, and each is well-formed by construction:

\begin{itemize}
  \item \tok{+\%}, \tok{-\%}, \tok{*\%} --- \emph{modular}: the result wraps
        modulo $2^N$.
  \item \tok{+|}, \tok{-|}, \tok{*|} --- \emph{saturating}: the result is
        clamped to the bounds of its type.
  \item \tok{+?}, \tok{-?}, \tok{*?}, \tok{as?} --- \emph{checked}: the
        operation shall be handled by an \lain{else} arm supplying a
        fallback value, propagating with \lain{return}, or
        \lain{panic}\-ing~(\diagcode{E067}).
\end{itemize}

\pnum
Within an \lain{unsafe} block the obligation is waived for every operation
lexically inside it, including the narrowing.  Responsibility passes to the
program.

\pnum
A conforming implementation that emits C99 shall compile the translation
output with \texttt{-fwrapv} or equivalent.  This is a requirement on the
\emph{translation output}, not a licence for the source language: it ensures
that the wrapping operators have their defined meaning and that no operation
the implementation has accepted becomes undefined behaviour downstream.

\begin{note}
  Unsigned arithmetic in ISO C is modular and signed overflow is undefined.
  Lain admits neither as a source-language behaviour for the plain operators:
  a program that would rely on either is rejected, and a program that wants
  one says which.  See \S\,\ref{sec:vra.boundary}.
\end{note}
