...
\texttt{malloc}/\texttt{free} pair as an \lain{extern func} with its row:
...
Slice (\lain{T[]})                            & Copy \texttt{\{len, data\}} pair & Always allowed \\
Raw pointer (\lain{*T})                       & Copy address & Always allowed \\
\end{longtable}

\begin{constraint}
  A linear value cannot be implicitly copied: transferring it (by assignment,
  argument, or return) without an explicit \lain{mov} is an implicit move and is
  \illformed{}~(\diagcode{E007}, \S\,\ref{sec:own.diag}).
\end{constraint}
...
No out-of-bounds access   & \diagcode{E085} (VRA proof failure) \\
