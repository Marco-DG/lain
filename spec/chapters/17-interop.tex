...
\begin{note}
  \textbf{Trust boundary --- I-016.}
  An \lain{extern func} declaration is a \textit{programmer assertion}
  that the referenced C function is pure (deterministic, no side
  effects, no access to mutable global state).  The Lain compiler
  cannot verify this property and trusts the declaration.  Misuse---
  for example, declaring \lain{libc\_rand} or \lain{malloc} as
  \lain{extern func}---is a programmer error, not a compile error, and
  may invalidate the guarantees that depend on purity (notably P4
  determinism for any \lain{func} that transitively calls the misdeclared
  extern).  When in doubt, use \lain{extern proc}.
\end{note}
