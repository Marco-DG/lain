...
def translate() -> None:
    path = "specs/Introducing Austral_ A Systems Language with Linear Types and Capabilities.html"
    with open(path, "r", encoding="utf-8") as f:
        content = f.read()

    # Generic text replacements
    content = content.replace("Austral", "Lain")
    content = content.replace("austral", "lain")
    
    # Let's replace code blocks block by block to make sure it's accurate
...
func store(borrow ptr Pointer_T, value T)
...
"""https://borretti.me/assets/card/introducing-lain.jpg"""
    ...
    for orig, rep in replacements:
        if orig not in content:
            print(f"NOT FOUND: {orig[:30]}...")
        content = content.replace(orig, rep)
    
    # In one of the replacements I used &amp; but it might be just & in the file, let me check.
    # I'll just write it. Wait, the HTML output had &[Path, R] so &amp; is how it's encoded in HTML maybe?
    # Wait, in the terminal output, I saw: function Read_File(path: &amp;[Path, R]): String;
    # Let me make sure that it's matched. I used exact copies of the view output. BUT the view output resolves &amp; sometimes or shows it. Wait, the view says `<line_number>: function Write_File(path: &[Path, R], content: String): Unit;`.
    # Wait, the view output shows `&`! Let's check:
    content = content.replace("&amp;", "&")

    # Re-apply replacements if needed for &amp; vs & issues
    for orig, rep in replacements:
        orig = orig.replace("&amp;", "&")
        content = content.replace(orig, rep)

    with open("specs/Introducing Lain_ A Systems Language with Linear Types and Capabilities.html", "w", encoding="utf-8") as f:
        f.write(content)
