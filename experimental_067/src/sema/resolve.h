...
void sema_build_scope(DeclList *decls, const char *module_path) {
    ...
    for (DeclList *dl = decls; dl; dl = dl->next) {
      ...
      case DECL_VARIABLE: {
        ...
        size_t clen = strlen(module_path) + 1 /* '_' */ + id->length + 1;
        char *cname = malloc(clen);
        snprintf(cname, clen, "%s_%s", module_path, raw);
      ...
      case DECL_FUNCTION: {
        ...
        size_t fclen = strlen(module_path) + 1 + id->length + 1;
        char *cnamef = malloc(fclen);
        snprintf(cnamef, fclen, "%s_%s", module_path, rawf);
      ...
      case DECL_STRUCT: {
        ...
        size_t sclen = strlen(module_path) + 1 + id->length + 1;
        char *cnames = malloc(sclen);
        snprintf(cnames, sclen, "%s_%s", module_path, raws);
      ...
      case DECL_ENUM: {
        ...
        char cnamet[256];
        size_t modlen = strlen(module_path);
        size_t max_rawt = sizeof(cnamet) - modlen - 2;
        if (max_rawt > 0) {
          snprintf(cnamet, sizeof(cnamet), "%s_%.*s", module_path, (int)max_rawt,
                   rawt);
        } else {
          // module_path is too long; just truncate
          memcpy(cnamet, module_path, sizeof(cnamet) - 1);
...
void sema_resolve_stmt(Stmt *s) {
  ...
  case STMT_VAR: {
    ...
    if (rhs) {
      ...
      sema_infer_expr(rhs);
      ty = rhs->type;           // capture the inferred type
      s->as.var_stmt.type = ty; // ← store it in your AST node
    }
