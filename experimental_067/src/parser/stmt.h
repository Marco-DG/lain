...
Stmt *parse_stmt(Arena* arena, Parser* parser)
{
    ...
    if (parser_match(TOKEN_IDENTIFIER)) {
        // peek the next token to decide whether this is:
        //  A) a declaration with a type:  IDENT IDENT ...  (name + type)
        //  B) an assignment:              IDENT '=' or '+=' etc.
        //  C) fallback expression statement
        ...
        switch (next.kind) {
            ...
            case TOKEN_CARET_EQUAL:
                return parse_assign_stmt(arena, parser);
            default:
                return parse_expr_stmt(arena, parser);
        }
    }
...
Stmt *parse_comptime_stmt(Arena* arena, Parser* parser)
{
    ...
    Type *type_annotation = NULL;
    if (parser_match(TOKEN_KEYWORD_MOV) || parser_match(TOKEN_IDENTIFIER) || parser_match(TOKEN_KEYWORD_COMPTIME)) {
        type_annotation = parse_type(arena, parser);
