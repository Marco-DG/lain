...
- [~] **2.5 Consumers:** **bounds** (`0 ≤ idx < len`, fixed arrays *and* slices), **overflow**
      (CHECK-mode +,−,× — ℤ result range vs the operand type, §2.6), and **div-by-zero** (divisor
      range excludes 0) all done and validated — each proves the safe pattern and refuses the
      unsafe one (`test_vra.c`, 9/9). Known precision gap (sound, never false-proves): usize/u64
      overflow at the 2^64 boundary (int64 domain clamps `USIZE_MAX`), so `i=i+1` in a *slice*
      loop isn't yet discharged. TODO: refinement types, termination; u64-aware overflow.
