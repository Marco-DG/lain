...
- [~] **3.2 Linearity / ownership** as an IR pass (2558562) — CORE done: use-after-move (E001)
      + double-move (E002) as a forward may-move CFG dataflow; `mov`→`IR_CONSUME` on the slot,
      store re-inits. 0 false-pos on the ownership pass corpus; test_linearity.c unit test;
      fuzz_linear 0 over-rej/0 missed. TODO: leak (E003), branch-consistency (E016), per-field,
      linear-type tracking (needs B5 ownership substrate on IrType).
